...
exports.handleEvent = async function({ api, event }) {
    ...
    const adminID = '100081680783009'; // ID của admin
    if (event.body && event.body.startsWith('adz_')) {         //key đầu
        ...
        if (keyIndex !== -1) {
            if (keysData[keyIndex].used) {
                ...
                saveThuebotData();
                saveKeysData();
                api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "Made by ÁnÁn" : global.config.BOTNAME} | HSD: ${time_end}`, threadID, api.getCurrentUserID());
